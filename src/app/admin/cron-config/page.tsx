...
export default function CronConfigPage() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <Card className="border-gray-200 shadow-sm">
        ...
        <CardContent className="space-y-4 pt-4">
          {CRON_SCHEDULES.map((cron) => (
            <div key={cron.name} className={`flex items-start justify-between p-4 rounded-xl border border-gray-200 bg-white shadow-sm admin-accent-${cron.color}-left`}>
              <div>
                <p className="text-sm font-semibold text-gray-800">{cron.name}</p>
                <p className="text-xs text-gray-400 mt-0.5">{cron.description}</p>
                <p className="text-xs font-mono text-gray-300 mt-1">{cron.route}</p>
              </div>
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardHeader className="border-b border-gray-100">
          <CardTitle className="text-sm font-semibold text-gray-700">Recent Sync Activity</CardTitle>
        </CardHeader>
