...
function CustomBuilder({
    ...
    for (const f of filters) {
      ...
      for (const [k, v] of Object.entries(f.params)) {
        if (typeof v === "boolean") {
          ...
          // Map param keys to API param names
  ...
  // Trigger filter change whenever filters change
  ...
  // Call onFilterChange whenever filters update
...
function ScreenerPageContent() {
  ...
  const fetchUrl = useMemo(() => {
    ...
    // Check if preset has active sliders with modified values
    ...
    if (presetDef?.sliders && presetDef.sliders.length > 0) {
      ...
      // Start with custom params derived from sliders
      ...
      // If slider values differ from defaults, use custom endpoint
