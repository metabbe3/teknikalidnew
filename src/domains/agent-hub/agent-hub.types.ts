export const AGENT_TYPES = [
  "seo_optimizer",
  "content_quality",
  "market_intel",
  "site_health",
  "stock_alert",
  "anomaly_detection",
  "community_sentiment",
  "user_engagement",
  "technical_screener",
  "gen_snapshots",
  "gen_sourced_news",
  "gen_trending_news",
  "gen_evergreen",
  "gen_faq",
  "growth_orchestrator",
  "seo_auditor",
  "content_expander",
  "internal_linker",
  "schema_builder",
  "growth_monitor",
] as const;

export type AgentType = (typeof AGENT_TYPES)[number];

export const JOB_STATUSES = ["pending", "running", "done", "failed", "cancelled"] as const;
export type JobStatus = (typeof JOB_STATUSES)[number];

export interface AgentJobPayload {
  [key: string]: unknown;
}

export interface AgentJobResult {
  [key: string]: unknown;
}

export interface AgentJobRow {
  id: string;
  agentType: AgentType;
  status: JobStatus;
  priority: number;
  payload: AgentJobPayload;
  result: AgentJobResult | null;
  error: string | null;
  attempts: number;
  maxAttempts: number;
  startedAt: Date | null;
  completedAt: Date | null;
  createdAt: Date;
  updatedAt: Date;
}

export interface AgentMeta {
  type: AgentType;
  label: string;
  description: string;
  icon: string;
  defaultSchedule: string;
  estimatedAiCalls: number;
}

export const AGENT_META: Record<AgentType, AgentMeta> = {
  seo_optimizer: {
    type: "seo_optimizer",
    label: "SEO Optimizer",
    description: "Analyzes articles for SEO improvements — meta tags, keywords, heading structure, internal links.",
    icon: "Search",
    defaultSchedule: "Daily at 08:00 WIB",
    estimatedAiCalls: 2,
  },
  content_quality: {
    type: "content_quality",
    label: "Content Quality",
    description: "Reviews articles for factual accuracy, readability, grammar, and clickbait detection.",
    icon: "FileCheck",
    defaultSchedule: "After article generation",
    estimatedAiCalls: 1,
  },
  market_intel: {
    type: "market_intel",
    label: "Market Intelligence",
    description: "Monitors RSS news, stock movements, and market sentiment for breaking events.",
    icon: "TrendingUp",
    defaultSchedule: "Every 30 min during market hours",
    estimatedAiCalls: 2,
  },
  site_health: {
    type: "site_health",
    label: "Site Health",
    description: "Checks database connectivity, page response times, cron schedules, and sitemap coverage.",
    icon: "Activity",
    defaultSchedule: "Every 6 hours",
    estimatedAiCalls: 0,
  },
  stock_alert: {
    type: "stock_alert",
    label: "Stock Alert",
    description: "Detects technical signal breakouts (RSI, MACD, SMA cross, Bollinger, Supertrend) and notifies users who follow or watchlist the stock.",
    icon: "Bell",
    defaultSchedule: "Every 30 min during market hours",
    estimatedAiCalls: 0,
  },
  anomaly_detection: {
    type: "anomaly_detection",
    label: "Anomaly Detection",
    description: "Detects unusual market patterns — price surges, volume spikes, RSI divergence, gorengan alerts.",
    icon: "AlertTriangle",
    defaultSchedule: "Every 30 min during market hours",
    estimatedAiCalls: 0,
  },
  community_sentiment: {
    type: "community_sentiment",
    label: "Community Sentiment",
    description: "Analyzes community posts per ticker using AI to generate bullish/bearish/neutral sentiment scores.",
    icon: "MessageSquare",
    defaultSchedule: "Every 6 hours",
    estimatedAiCalls: 1,
  },
  user_engagement: {
    type: "user_engagement",
    label: "User Engagement",
    description: "Sends personalized notifications from watchlist/stock follow data, re-engages inactive users.",
    icon: "Users",
    defaultSchedule: "Daily 08:00 WIB",
    estimatedAiCalls: 0,
  },
  technical_screener: {
    type: "technical_screener",
    label: "Technical Screener",
    description: "Scans all IDX stocks for bullish/bearish technical setups (Golden Cross, RSI bounce, MACD cross, volume spike, etc.) and posts a daily screener summary to community.",
    icon: "BarChart3",
    defaultSchedule: "Daily 09:30 WIB (weekdays)",
    estimatedAiCalls: 1,
  },
  gen_snapshots: {
    type: "gen_snapshots",
    label: "Generate Snapshots",
    description: "Generates daily stock snapshot articles for all tracked stocks. Weekdays only (needs fresh market data).",
    icon: "Camera",
    defaultSchedule: "Weekdays 10:00 WIB (after EOD sync)",
    estimatedAiCalls: 0,
  },
  gen_sourced_news: {
    type: "gen_sourced_news",
    label: "Generate Sourced News",
    description: "Scrapes RSS from trusted financial sources, filters via AI, and generates news articles. Most AI-intensive step.",
    icon: "Newspaper",
    defaultSchedule: "3x daily (morning, lunch, afternoon)",
    estimatedAiCalls: 5,
  },
  gen_trending_news: {
    type: "gen_trending_news",
    label: "Generate Trending News",
    description: "Uses AI to identify trending financial topics and generate original analysis articles.",
    icon: "TrendingUp",
    defaultSchedule: "3x daily (morning, lunch, afternoon)",
    estimatedAiCalls: 5,
  },
  gen_evergreen: {
    type: "gen_evergreen",
    label: "Generate Evergreen Analysis",
    description: "Generates IDX40 technical analysis articles for stocks in the queue. Processes a small batch each run.",
    icon: "FileText",
    defaultSchedule: "Daily",
    estimatedAiCalls: 2,
  },
  gen_faq: {
    type: "gen_faq",
    label: "Generate FAQ",
    description: "Generates FAQ content for the site based on trending topics and user queries.",
    icon: "HelpCircle",
    defaultSchedule: "Daily",
    estimatedAiCalls: 3,
  },
  growth_orchestrator: {
    type: "growth_orchestrator",
    label: "Growth Orchestrator (Mandor)",
    description: "Analyzes site health, content gaps, SEO issues, then dispatches specialist agents to fix them. The brain of growth automation.",
    icon: "General",
    defaultSchedule: "Every 6 hours",
    estimatedAiCalls: 2,
  },
  seo_auditor: {
    type: "seo_auditor",
    label: "SEO Auditor",
    description: "Scans articles and pages for missing meta descriptions, duplicate titles, thin content, broken internal links, and missing schema markup.",
    icon: "SearchCheck",
    defaultSchedule: "After orchestrator dispatch",
    estimatedAiCalls: 1,
  },
  content_expander: {
    type: "content_expander",
    label: "Content Expander",
    description: "Generates glossary terms, educational articles, and FAQ entries to fill content gaps. Targets high-value keywords.",
    icon: "FilePlus",
    defaultSchedule: "After orchestrator dispatch",
    estimatedAiCalls: 3,
  },
  internal_linker: {
    type: "internal_linker",
    label: "Internal Linker",
    description: "Automatically adds contextual internal links between related articles, stock pages, and FAQ entries to boost SEO.",
    icon: "Link",
    defaultSchedule: "After orchestrator dispatch",
    estimatedAiCalls: 1,
  },
  schema_builder: {
    type: "schema_builder",
    label: "Schema Builder",
    description: "Injects JSON-LD structured data (Article, FAQPage, Organization) into articles and pages for rich search results.",
    icon: "Code",
    defaultSchedule: "After orchestrator dispatch",
    estimatedAiCalls: 0,
  },
  growth_monitor: {
    type: "growth_monitor",
    label: "Growth Monitor",
    description: "Tracks article count, indexation status, content coverage gaps, and reports growth metrics over time.",
    icon: "TrendingUp",
    defaultSchedule: "After orchestrator dispatch",
    estimatedAiCalls: 0,
  },
};

export interface AgentConfigRow {
  id: string;
  agentType: AgentType;
  isEnabled: boolean;
  systemPrompt: string | null;
  scheduleCron: string | null;
  scheduleMeta: Record<string, unknown> | null;
  config: Record<string, unknown> | null;
  updatedAt: Date;
  createdAt: Date;
}

export interface SchedulePreset {
  value: string;
  label: string;
  cron: string | null;
}

export const SCHEDULE_PRESETS: SchedulePreset[] = [
  { value: "every_30m", label: "Every 30 minutes", cron: "*/30 * * * *" },
  { value: "hourly", label: "Hourly", cron: "0 * * * *" },
  { value: "every_6h", label: "Every 6 hours", cron: "0 1,7,13,19 * * *" },
  { value: "daily_08", label: "Daily 08:00 WIB", cron: "0 1 * * *" },
  { value: "daily_weekdays_08", label: "Daily 08:00 WIB (weekdays)", cron: "0 8 * * 1-5" },
  { value: "market_hours_30", label: "Every 30 min (market hours)", cron: "*/30 9-14 * * 1-5" },
  { value: "chained", label: "After article generation (chained)", cron: null },
  { value: "disabled", label: "Disabled", cron: null },
];

export interface AgentHubStats {
  totalJobs: number;
  pendingJobs: number;
  runningJobs: number;
  completedJobs: number;
  failedJobs: number;
  agents: {
    type: AgentType;
    label: string;
    lastRun: Date | null;
    lastStatus: JobStatus | null;
    successRate: number;
    totalRuns: number;
  }[];
  recentJobs: AgentJobRow[];
  configs?: AgentConfigRow[];
}
